...
while(True):
  ...
  try:
    ...
    pixel_filter = 20
    #percentile function cnverts the value into the scalar quantity
    min_pixel = np.percentile(image_bw_resized_inverted, pixel_filter)
    #clip is used to limit the values
    image_bw_resized_inverted_scaled = np.clip(image_bw_resized_inverted-min_pixel, 0, 255)
    #gets the maximum of the given numbers
    max_pixel = np.max(image_bw_resized_inverted)
    #np.asarray()it converts given values into array (we have to change the data to the array format)
    ...
    # Display the resulting in frame
    cv2.imshow('frame',gray)
    if cv2.waitKey(1) & 0xFF == ord('q'):
      break
      #if there is any error pass it to the nxt
